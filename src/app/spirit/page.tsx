  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="p-2.5 rounded-xl bg-rose-500/10"><Users size={22} className="text-rose-400" /></div>
        <h1 className="text-xl font-medium text-zinc-200">精神</h1>
      </div>
        ...
        <CardContent className="p-4">
          <div className="flex gap-2">
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="姓名"
              className="w-36 bg-zinc-800/50 border-zinc-700/50 rounded-xl text-sm" />
            ...
            <Button onClick={addContact} size="sm" disabled={!name.trim()} className="shrink-0 rounded-xl text-sm">
      ...
      <div className="space-y-2">
        ...
        {contacts.length === 0 && <p className="text-sm text-zinc-500 text-center py-8">还没有联系人</p>}
      </div>
    </div>
  );
}
      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {contact.tags && <div className="flex gap-1">{contact.tags.split(",").map((t, i) => <Badge key={i} variant="outline" className="text-[10px]">{t.trim()}</Badge>)}</div>}
            {contact.last_contact && (
              <span className={cn("text-xs", daysSince > 30 ? "text-amber-400" : "text-zinc-500")}>{daysSince}天前联系</span>
            )}
          </div>
          <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (v) fetchLogs(); }}>
            <DialogTrigger className="inline-flex items-center gap-1.5 text-sm text-zinc-400 hover:text-zinc-200 py-1.5 px-3 rounded-xl transition-colors">
              <MessageCircle size={16} />联系记录
            </DialogTrigger>
            <DialogContent className="bg-zinc-900 border-zinc-700 max-w-md rounded-2xl">
              <DialogHeader><DialogTitle className="text-base text-zinc-200">{contact.name} · 联系记录</DialogTitle></DialogHeader>
              <div className="space-y-3">
                <div className="flex gap-2">
                  <Input value={summary} onChange={(e) => setSummary(e.target.value)} placeholder="这次聊了什么..."
                    className="bg-zinc-800/50 border-zinc-700/50 rounded-xl text-sm" onKeyDown={(e) => e.key === "Enter" && addLog()} />
                  <Button onClick={addLog} size="sm" disabled={!summary.trim()} className="rounded-xl text-sm">保存</Button>
                </div>
                <div className="space-y-1 max-h-48 overflow-auto">
                  {logs.map((l) => (
                    <div key={l.id} className="text-sm p-2.5 rounded-lg bg-zinc-800/40">
